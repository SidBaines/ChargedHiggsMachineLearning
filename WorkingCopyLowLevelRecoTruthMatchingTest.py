...
for dsid in [510115, 510118, 510123]:
    ...
    plt.hist(m*1e-3, histtype='step', label=val_metrics_MCWts.DSID_MASS_MAPPING[dsid],bins=bins)
plt.legend()
# plt.savefig('tmp.pdf')
plt.show()
plt.close('all')
...
true_inclusion = x[...,-1]
for i in range(3):
    mapping = ['None', 'lvbb', 'qqbb']
    print(f'Truth: {mapping[y.argmax(dim=-1)[i].item()]} {train_metrics_MCWts.DSID_MASS_MAPPING[dsids[i].item()]}TeV')
    print(f'      #e  #m  #n  #l  #s')
    for inclusion, inctype in zip([true_inclusion, pred_inclusion], ['true', 'pred']):
        print(inctype, end=': ')
        ...
        if N_CTX==6: # all ljets are type==3
            num_ljets=(((types == 3).to(int) * (inclusion>0).to(int))).sum(dim=-1)
        elif N_CTX==7: # We separated ljets into xbb type==5 and not-xbb type==3
        ...
        # print((((types.to(int) == 3) * pred_inclusion)).sum(dim=-1).sum())
        # print((((types.to(int) == 5) * pred_inclusion)).sum(dim=-1).sum())
        print(f"  {num_electrons[i].item()}", end=" ")
        print(f"  {num_muons[i].item()}", end=" ")
        print(f"  {num_neutrinos[i].item()}", end=" ")
        print(f"  {num_ljets[i].item()}", end=" ")
        print(f"  {num_sjets[i].item()}", end=" ")
        print("")
    print("------------------------------------------------")

print(((pred_inclusion==true_inclusion)|types==(N_CTX-1))[:3])
print(pred_inclusion[:3])
print(true_inclusion[:3])
print(types[:3])
print(y.argmax(dim=-1)[:3])
...
true_inclusion = x[...,-1]
reco_inclusion = x[...,-2]
inclusion=true_inclusion

def check_valid(types, inclusion):
    num_electrons=(((types == 0).to(int) * (inclusion>0).to(int))).sum(dim=-1)
    num_muons=(((types == 1).to(int) * (inclusion>0).to(int))).sum(dim=-1)
    num_neutrinos=(((types == 2).to(int) * (inclusion>0).to(int))).sum(dim=-1)
    if N_CTX==6: # all ljets are type==3
...
print(f"{sum(check_valid(types, true_inclusion))}/{len(types)}")
print(f"{sum(check_valid(types, pred_inclusion))}/{len(types)}")
print(f"{sum(check_valid(types, reco_inclusion))}/{len(types)}")
# %%
