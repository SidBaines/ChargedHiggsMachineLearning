...
import numpy as np
from utils.utils import read_file, check_category
import os
import torch
...
running_stats = {
    'lvbb':RunningStats(N_Real_Vars+1),
    'qqbb':RunningStats(N_Real_Vars+1),
}
...
def process_single_file(filepath, max_n_objs, shuffle_objs, target_channel):
    ...
    if target_channel == 'lvbb':
        wrong_reco_removal = (selection_category != 0) & (selection_category != 8) & (selection_category != 10)
    elif target_channel == 'qqbb':
        wrong_reco_removal = (selection_category != 3) & (selection_category != 9)
    else:
        assert(False)
    combined_removal = (no_ljet | selection_removals | low_MET | mH_cut | uncertain_cut | keeping_all_truth_removal | successful_truth_match_removals | OnlyLjetBoson_removal | wrong_reco_removal | truth_removal)
