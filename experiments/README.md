...
| `resolved_r1_baseline_probes.py` | resolved R1 | fresh-slice resolved-qqbb baseline/probes: weak two-sjet bookkeeping, `mjj` alone weak for claims/margins, context/relative-hardness features dominate observational probes; path hints in b2h3/b0h3/b1h3 sjet-sjet attention. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r2_pair_swaps.py` | resolved R2 | real-event W-sjet-pair swaps: donor-placebo shows transplant disruption; relative-pT changes have large causal effects under tight `mjj` matching; large `mjj` changes under relative-pT matching do not give a clean pair-mass-window effect. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r3_angle_sweep.py` | resolved R3 | synthetic pair-angle sweep: co-rotation has small direction/recoil effect; sweeping W-sjet opening angle changes median `mjj` ~95→256 GeV with almost no lep/ν verdict effect, though two-W-sjet joint claiming weakens mildly at large opening angle. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r4_sjet_edge_knockout.py` | resolved R4 | direct W-sjet↔W-sjet edge KO: `b2h3` subleading-query→leading-key causally supports the subleading sjet's own W claim and two-sjet joint claiming, but barely affects lep/ν channel verdict; `b0h3` high attention is causally null. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r5_success_strata.py` | resolved R5 | success-stratified resolved analysis: clean-exclusive events reveal a strong causal `b2h3` subleading→leading pair-binding circuit, but cutting it still does not move the lep/ν verdict; partial/error events are not rescued by direct pair-edge behavior. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r6_failure_rescue.py` | resolved R6 | failure rescue: clean-success `b2h2+b1h3` scalar overwrites at lep/ν rescue all channel-error events, but barely restore clean two-sjet object labels; pair-hardness boosts partially rescue channel errors and partials but are blunt/OOD and can degrade clean successes. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r7_sjet_context_queries.py` | resolved R7 | sjet context-query test: clean W-sjet labels causally depend on direct reads from lep/ν (`b0h0`, `b1h3`, `b2h2`) and H/large jets (`b0h1`, `b1h1`); partner `b2h3` binds the subleading W-sjet; non-W sjets read W-sjets suppressively. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r8_message_content.py` | resolved R8 | path-message content decomposition: saves attention mass and scalar bottleneck contributions for lead/sub/non-W sjet queries to lep/ν, large jets, partner/W-sjets, and other sjets; probes messages against named physics features and outcomes. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
| `resolved_r9_partner_message_patch.py` | resolved R9 | causal path-message replacement: patches only the `b2h3` subleading-query→leading-key partner scalar contribution with clean/failure/shuffled/formula values to test sufficiency and formula faithfulness. Results: `docs/logs/2026-06-12_resolved-qqbb-w-circuit.md` |
