...
num_blocks_variable=4
num_clasifierlayers_variable=8
model_cfg = {'include_mlp':True, 'd_model': 200, 'd_attn':None, 'd_mlp': 400, 'num_blocks':num_blocks_variable, 'dropout_p': 0.0, "embedding_size":N_CTX, "num_heads":4}
num_epochs = 30
...
if 1:
    ...
    for batch_idx in range(orig_len_val_dataloader):
        ...
        for dsid in val_dataloader.current_indices:
            ...
            for cat in range(6):
                cat_sel = cats == cat
                num_events[dsid][cat] += MCWts[dsid_sel & cat_sel.to(device)].sum().item()

    # print(num_events)
    # wefwefwef
# %%
for dsid in [510115, 510117, 510121, 510124]:
    print('& total', end='')
    for cat in range(6):
        print(f'& {int(num_events[dsid][cat])*2:5d}', end=' ')
    print()
print('--------------------')
print('& total', end='')
for cat in range(6):
    print(f'& {int(sum([num_events[dsid][cat] for dsid in num_events.keys()]))*2:5d}', end=' ')
print()
...
if 1: # 
    ...
    modelfile="/users/baines/Code/ChargedHiggs_ExperimentalML/output/20250705-184750_TrainingOutput/models/Nplits2_ValIdx0/chkpt29_164490.pth" # DSSAR d_model=32,    d_head=8,    n_layers=8,    n_heads=8,    d_mlp=128,
...
if 0:
    # Validation phase on normal data
    model.eval()
    with torch.no_grad():
        val_dataloader._reset_indices()
        orig_len_val_dataloader=len(val_dataloader)
        ...
        for batch_idx in range(orig_len_val_dataloader):
            if (batch_idx % 20) == 0:
                print(f"Validation batch {batch_idx}/{orig_len_val_dataloader}")
            batch = next(val_dataloader)
            x, y, w, types, dsids, mqq, mlv, MCWts, mHs = batch.values()
    ...
    scores['normal'] = val_metrics_MCWts.compute_and_log(0, prefix="val_MC", log_level=5, save=False, commit=False, calc_all=True)
...
# %%
# Validation phase on data where the lepton and neutrino are in the W decay, WITH and WITHOUT the angle between them rotated
from utils.utils import Rotate4VectorPhiEta, Get_PtEtaPhiM_fromXYZT, deltaR, GetXYZT_FromPtEtaPhiM, check_category
model.eval()
# for modification in ['none', 'rotate_lep_to_nu', 'rotate_nu_to_lep', 'rotate_lep_away_from_nu', 'rotate_nu_away_from_lep', 'rotate_ljet_to_lep_lvJOnly', 'rotate_ljet_away_from_lep_lvJOnly', 'rotate_lep_to_ljet_lvJOnly', 'rotate_lep_away_from_ljet_lvJOnly', 'remove_all_nonpresent_jets']:
# for modification in ['rotate_ljet_to_lep_lvJOnly', 'rotate_ljet_away_from_lep_lvJOnly', 'rotate_lep_to_ljet_lvJOnly', 'rotate_lep_away_from_ljet_lvJOnly']:
for modification in ['remove_all_nonpresent_sjets', 'remove_all_nonpresent_ljets']:
# for modification in ['none']:
    with torch.no_grad():
        val_dataloader._reset_indices()
        val_metrics_MCWts = HEPMetrics(N_CTX-1, max_n_objs_to_read, is_categorical=IS_CATEGORICAL, num_categories=3, max_bkg_levels=[100, 200], max_buffer_len=int(val_dataloader.get_total_samples()), total_weights_per_dsid=val_dataloader.weight_sums, signal_acceptance_levels=[100, 500, 1000, 5000])
        orig_len_val_dataloader=len(val_dataloader)
        ...
        for batch_idx in range(orig_len_val_dataloader):
            ncheck= 20
            if (batch_idx % ncheck) == (ncheck-1):
                print(f"Validation batch {batch_idx}/{orig_len_val_dataloader}")
                # break
            batch = next(val_dataloader)
            x, y, w, types, dsids, mqq, mlv, MCWts, mHs = batch.values()
            # is_lvbb_selection = x[...,0,-1] == 3
            neutrino_4vec = x[...,0,:4]
            neutrino_pt, neutrino_eta, neutrino_phi, neutrino_m = Get_PtEtaPhiM_fromXYZT(neutrino_4vec[...,0], neutrino_4vec[...,1], neutrino_4vec[...,2], neutrino_4vec[...,3], use_torch=True)
            lepton_4vec = x[...,1,:4]
            lepton_pt, lepton_eta, lepton_phi, lepton_m = Get_PtEtaPhiM_fromXYZT(lepton_4vec[...,0], lepton_4vec[...,1], lepton_4vec[...,2], lepton_4vec[...,3], use_torch=True)
            delta_r, delta_phi, delta_eta = deltaR(neutrino_4vec.detach().cpu().numpy(), lepton_4vec.detach().cpu().numpy())
            if modification  == 'none':
                pass
            elif modification  == 'rotate_lep_to_nu':
                rotated_lepton = GetXYZT_FromPtEtaPhiM(lepton_pt, lepton_eta, neutrino_phi, lepton_m, use_torch=True)
                x[:,0,0] = rotated_lepton[0]
                x[:,0,1] = rotated_lepton[1]
                x[:,0,2] = rotated_lepton[2]
                x[:,0,3] = rotated_lepton[3]
            elif modification  == 'rotate_nu_to_lep':
                rotated_neutrino = GetXYZT_FromPtEtaPhiM(neutrino_pt, neutrino_eta, lepton_phi, neutrino_m, use_torch=True)
                x[:,0,0] = rotated_neutrino[0]
                x[:,0,1] = rotated_neutrino[1]
                x[:,0,2] = rotated_neutrino[2]
                x[:,0,3] = rotated_neutrino[3]
            elif modification  == 'rotate_lep_away_from_nu':
                rotated_lepton = GetXYZT_FromPtEtaPhiM(lepton_pt, lepton_eta, neutrino_phi+np.pi, lepton_m, use_torch=True)
                x[:,0,0] = rotated_lepton[0]
                x[:,0,1] = rotated_lepton[1]
                x[:,0,2] = rotated_lepton[2]
                x[:,0,3] = rotated_lepton[3]
            elif modification  == 'rotate_nu_away_from_lep':
                rotated_neutrino = GetXYZT_FromPtEtaPhiM(neutrino_pt, neutrino_eta, lepton_phi+np.pi, neutrino_m, use_torch=True)
                x[:,0,0] = rotated_neutrino[0]
                x[:,0,1] = rotated_neutrino[1]
                x[:,0,2] = rotated_neutrino[2]
                x[:,0,3] = rotated_neutrino[3]
            elif modification == 'rotate_ljet_to_lep_lvJOnly':
                lvJOnly = check_category(types, #[batch object]
                   x[...,-1], # [batch object]
                   N_CTX-1, # int
                   use_torch=True, # bool
                   ) == 4
                x, y, w, types, dsids, mqq, mlv, MCWts, mHs = x[lvJOnly], y[lvJOnly], w[lvJOnly], types[lvJOnly], dsids[lvJOnly], mqq[lvJOnly], mlv[lvJOnly], MCWts[lvJOnly], mHs[lvJOnly]
                lepton_4vec = x[...,1,:4]
                lepton_pt, lepton_eta, lepton_phi, lepton_m = Get_PtEtaPhiM_fromXYZT(lepton_4vec[...,0], lepton_4vec[...,1], lepton_4vec[...,2], lepton_4vec[...,3], use_torch=True)
                batch_inds, lJetHCand_locs = torch.where(x[...,-1] == 1)
                ljetHcand_4vec = x[batch_inds, lJetHCand_locs, :4]
                ljetHcand_pt, ljetHcand_eta, ljetHcand_phi, ljetHcand_m = Get_PtEtaPhiM_fromXYZT(ljetHcand_4vec[...,0], ljetHcand_4vec[...,1], ljetHcand_4vec[...,2], ljetHcand_4vec[...,3], use_torch=True)
                rotated_ljetHcand = GetXYZT_FromPtEtaPhiM(ljetHcand_pt, ljetHcand_eta, lepton_phi, ljetHcand_m, use_torch=True)
                x[batch_inds, lJetHCand_locs, 0] = rotated_ljetHcand[0]
                x[batch_inds, lJetHCand_locs, 1] = rotated_ljetHcand[1]
                x[batch_inds, lJetHCand_locs, 2] = rotated_ljetHcand[2]
                x[batch_inds, lJetHCand_locs, 3] = rotated_ljetHcand[3]
            elif modification == 'rotate_ljet_away_from_lep_lvJOnly':
                lvJOnly = check_category(types, #[batch object]
                   x[...,-1], # [batch object]
                   N_CTX-1, # int
                   use_torch=True, # bool
                ...
                ljetHcand_4vec = x[batch_inds, lJetHCand_locs, :4]
                ljetHcand_pt, ljetHcand_eta, ljetHcand_phi, ljetHcand_m = Get_PtEtaPhiM_fromXYZT(ljetHcand_4vec[...,0], ljetHcand_4vec[...,1], ljetHcand_4vec[...,2], ljetHcand_4vec[...,3], use_torch=True)
                rotated_ljetHcand = GetXYZT_FromPtEtaPhiM(ljetHcand_pt, ljetHcand_eta, lepton_phi+np.pi, ljetHcand_m, use_torch=True)
                x[batch_inds, lJetHCand_locs, 0] = rotated_ljetHcand[0]
                x[batch_inds, lJetHCand_locs, 1] = rotated_ljetHcand[1]
                x[batch_inds, lJetHCand_locs, 2] = rotated_ljetHcand[2]
                x[batch_inds, lJetHCand_locs, 3] = rotated_ljetHcand[3]
            elif modification == 'rotate_lep_to_ljet_lvJOnly':
                lvJOnly = check_category(types, #[batch object]
                   x[...,-1], # [batch object]
                   N_CTX-1, # int
                   use_torch=True, # bool
                ...
                ljetHcand_4vec = x[batch_inds, lJetHCand_locs, :4]
                ljetHcand_pt, ljetHcand_eta, ljetHcand_phi, ljetHcand_m = Get_PtEtaPhiM_fromXYZT(ljetHcand_4vec[...,0], ljetHcand_4vec[...,1], ljetHcand_4vec[...,2], ljetHcand_4vec[...,3], use_torch=True)
                rotated_lepton = GetXYZT_FromPtEtaPhiM(lepton_pt, lepton_eta, ljetHcand_phi, lepton_m, use_torch=True)
                x[:,0,0] = rotated_lepton[0]
                x[:,0,1] = rotated_lepton[1]
                x[:,0,2] = rotated_lepton[2]
                x[:,0,3] = rotated_lepton[3]
            elif modification == 'rotate_lep_away_from_ljet_lvJOnly':
                lvJOnly = check_category(types, #[batch object]
                   x[...,-1], # [batch object]
                   N_CTX-1, # int
                   use_torch=True, # bool
                ...
                ljetHcand_4vec = x[batch_inds, lJetHCand_locs, :4]
                ljetHcand_pt, ljetHcand_eta, ljetHcand_phi, ljetHcand_m = Get_PtEtaPhiM_fromXYZT(ljetHcand_4vec[...,0], ljetHcand_4vec[...,1], ljetHcand_4vec[...,2], ljetHcand_4vec[...,3], use_torch=True)
                rotated_lepton = GetXYZT_FromPtEtaPhiM(lepton_pt, lepton_eta, ljetHcand_phi+np.pi, lepton_m, use_torch=True)
                x[:,0,0] = rotated_lepton[0]
                x[:,0,1] = rotated_lepton[1]
                x[:,0,2] = rotated_lepton[2]
                x[:,0,3] = rotated_lepton[3]
            elif modification == 'remove_all_nonpresent_jets':
                is_jet = types > 2
                is_nonpresent = x[...,-1] == 0
                has_nonpresent_jet = (is_jet & is_nonpresent).any(dim=1)
                # x, y, w, types, dsids, mqq, mlv, MCWts, mHs = x[has_nonpresent_jet], y[has_nonpresent_jet], w[has_nonpresent_jet], types[has_nonpresent_jet], dsids[has_nonpresent_jet], mqq[has_nonpresent_jet], mlv[has_nonpresent_jet], MCWts[has_nonpresent_jet], mHs[has_nonpresent_jet]
                # Get final occurrence of the nonpresent jet along the dim=1
                batch_inds, nonpresent_jet_locs = torch.where(is_jet & is_nonpresent)
                types[batch_inds, nonpresent_jet_locs] = N_CTX-1
                x[batch_inds, nonpresent_jet_locs,:] = 0
            elif modification == 'remove_all_nonpresent_sjets':
                is_jet = types == 4
                is_nonpresent = x[...,-1] == 0
                has_nonpresent_jet = (is_jet & is_nonpresent).any(dim=1)
                batch_inds, nonpresent_jet_locs = torch.where(is_jet & is_nonpresent)
                types[batch_inds, nonpresent_jet_locs] = N_CTX-1
                x[batch_inds, nonpresent_jet_locs,:] = 0
            elif modification == 'remove_all_nonpresent_ljets':
                is_jet = types == 3
                is_nonpresent = x[...,-1] == 0
                has_nonpresent_jet = (is_jet & is_nonpresent).any(dim=1)
                batch_inds, nonpresent_jet_locs = torch.where(is_jet & is_nonpresent)
                types[batch_inds, nonpresent_jet_locs] = N_CTX-1
                x[batch_inds, nonpresent_jet_locs,:] = 0
            elif modification == 'multiply_pt_1sigma':
                all_pt, all_eta, all_phi, all_m = Get_PtEtaPhiM_fromXYZT(x[...,0], x[...,1], x[...,2], x[...,3], use_torch=True)
                # lepton_pt = 
                torch.randn(all_pt,)
                raise NotImplementedError
            elif modification == 'remove_lower_pt_nonpresent_jet':
                raise NotImplementedError
            #     is_jet = types > 2
            #     is_nonpresent = x[...,-1] == 0
            #     has_nonpresent_jet = (is_jet & is_nonpresent).any(dim=1)
    ...
    # scores[modification] = val_metrics_MCWts.compute_and_log(0, prefix="val_MC", log_level=5, save=False, commit=False, calc_all=True)
    scores[modification] = val_metrics_MCWts.compute_and_log(0, prefix="val_MC", log_level=5, save=False, commit=False, calc_all=False)
...
for mod in scores.keys():
    print(f'{mod:35s}', end=' &')
    for cat in range(6):
        print(f' & {scores[mod][f"val_MC/PerfectRecoPct_all_cat{cat}"]:.4f}', end=' ')
    print()
