# 2026-06-12 — E4 faithfulness: readout-level verdict compression with two formulas
...
## E4 part 1 — symbolic readout fitting (`experiments/h1/e4_wire_extraction.py`)
...
## E4 part 2 — in-network readout replacement (`experiments/h1/e4_wire_replacement.py`)

The wires are OVERWRITTEN at the ν and lepton positions with the formula values
(wave2 override-hook math; everything else neural). Codex follow-up
(`2026-06-12_codex-analysis-fixes.md`) corrected the mean-control to use the true
train-set wire means and made formula-domain failures explicit; the simple c=8
b2h2 formula has one held-out sqrt-domain failure, filled by default with the true
train-set mean. Corrected held-out metrics:

| condition | ν == intact | ν == truth | P(ν=lep) |
|---|---|---|---|
| intact | 1.0000 | 0.9551 | 0.9941 |
| replace b1h3 (c=6 formula) | 0.9890 | 0.9534 | 0.9919 |
| replace b2h2 (c=8 formula) | 0.9634 | 0.9375 | 0.9915 |
| **replace BOTH (simple, c=6+8)** | **0.9553** | **0.9309** | 0.9880 |
| replace BOTH (c=22 formulas) | 0.9636 | 0.9402 | 0.9897 |
| mean-control BOTH (true train-set means; info destroyed) | 0.6951 | 0.6941 | 0.9851 |

**Headline: two simple readout-level formulas — one of them a single tanh of
pT(lepW)/HT — can be spliced into the two dominant verdict wires while keeping
95.5% of held-out decisions (96.4% with the complexity-22 versions) at a 2.4-pt
truth-accuracy cost; the true-mean control floor is 69.5% (the lvbb base rate), so
the formulas recover most, but not all, of the wire information.**
...
- The default fit/test split is not fully prospective with respect to feature
  discovery: the 15 features were chosen using earlier H1 findings. Treat this as
  post-hoc compression until rerun on a fresh slice or under a nested protocol.
