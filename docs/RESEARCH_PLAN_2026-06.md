...
- 2026-06-16 — **Fixed the qqbb ROC-AUC bug** (`compute_auc` hardcoded the lvbb prob column for both channels) and **regenerated the ch.7 classification figures** on heppc402 (qqbb recomputed, lvbb reused; y-axis capped at 1.0; figures relaid 4×1→2×2, thesis `\note`s removed). Fix `79d3312` (this repo) + thesis `6425031`. Wrote the cluster/data how-to: [`CLUSTER_AND_DATA_GUIDE.md`](CLUSTER_AND_DATA_GUIDE.md). → [`logs/2026-06-16_qqbb-roc-bug-and-regen.md`](logs/2026-06-16_qqbb-roc-bug-and-regen.md). (Closes the ch.7 qqbb-ROC item under Phase 4.)
