...
- 2026-06-12 — **E4 faithfulness MILESTONE**: verdict wires extracted to closed-form physics formulas (b1h3 = −4.57·tanh(pT(lepW)/HT)+1.25, R²=0.84) and spliced back in-network: **95.5% of decisions kept with the two simple formulas** (floor 69.5%); jet-side claim formula = next E4 stage. Also: LR sweep (legacy recipe undertrains ~3pts; suite recipe = 1e-3 cosine ramp), Phase-2 suite queue launched (26 runs), formation crisis confirmed real under sane schedule. → [`logs/2026-06-12_faithfulness-e4.md`](logs/2026-06-12_faithfulness-e4.md).
