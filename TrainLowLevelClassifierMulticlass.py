...
config = {
        ...
        "wandb":True,
        "name":"_"+timeStr+"_LowLevel_"+name_mapping[MODEL_ARCH]+"_combined"+f"_Only{str(KEEP_DSID)}"*(KEEP_DSID is not None)+f"_Min{str(MIN_DSID)}"*(MIN_DSID is not None)+f"_Max{str(MAX_DSID)}"*(MAX_DSID is not None)+f"_Excl{str(EXCL_DSID)}"*(EXCL_DSID is not None)+"_NoEventNumbers"*(not USE_EVENT_NUMBERS),
        "weight_decay":1e-10,
    }
...
for epoch in range(num_epochs):
    ...
    if ((epoch % SAVE_MODEL_EVERY) == (SAVE_MODEL_EVERY-1)):
        try:
            modelSaveDir = "%s/models/%s_Nplits%d_ValIdx%d/"%(saveDir, "combined", n_splits, validation_split_idx)
