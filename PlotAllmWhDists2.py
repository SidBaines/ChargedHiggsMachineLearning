...
for dir in modeltags:
    ...
    if HighLevel:
        for channel in channels:
            ...
            selectionAft = bkg_sel & (metric_tracker.all_probs[:,0]<thresholds[200][mHCut][channel])
            if mHCut=='mHCut':
        ...
        for channel in channel_labels.keys():
            ...
            selectionAft = bkg_sel & this_sig_sel & (metric_tracker.all_probs[:,0]<thresholds[200][mHCut][channel])
            if mHCut=='mHCut':
                selectionAft = selectionAft & ((metric_tracker.all_mHs > 95e3) & (metric_tracker.all_mHs < 140e3))
            elif mHCut=='NoCut':
                pass
...
if mHCut == 'mHCut':
    selections = {
        "lvbb Original": lambda events: (events["MET"] > 30e3) & ((events["selection_category"]==0)|(events["selection_category"]==8)|(events["selection_category"]==10)) & ((events["mH"]>95)&(events["mH"]<140)) & (events["NN_lvbb_Final_Combined"]>0.86),
        "qqbb Original": lambda events: (events["MET"] > 30e3) & ((events["selection_category"]==3)|(events["selection_category"]==9)) & ((events["mH"]>95)&(events["mH"]<140)) & (events["NN_qqbb_Final_Combined"]>0.395),
...
if 1:
    ...
    for sel_name, data in hist_data.items():
        ...
        plt.hist(data, weights=weight_data[sel_name], bins=bins, density=density, alpha=0.8, label=sel_name, histtype='step', color='black')
        plt.legend(prop={'size':legsize})

plt.suptitle('Density distributions of background')
plt.tight_layout()
plt.savefig(f'mWhDists8_{mHCut}.pdf')
plt.show()
plt.close()
...
nbins=70
bins=nbins
bins=np.arange(nbins+1)/nbins*3500
channel_labels = {'lvbb':1, 'qqbb':2}
BASE_DIR='/data/atlas/baines/TmpCommonModelResults/'
colors={'20250211-103234_lvbb':'r',
        '20250217-142417_qqbb':'r', 
...
for sig_dsid in range(510115, 510125):
    ...
    plt.tight_layout()
    plt.savefig(f'mWhDists5_{sig_dsid}_{mHCut}.pdf')
    plt.show()
    plt.close()
