...
from utils import get_num_btags, Get_PtEtaPhiM_fromXYZT, GetXYZT_FromPtEtaPhiM, GetXYZT_FromPtEtaPhiE, Rotate4VectorPhi, Rotate4VectorEta, Rotate4VectorPhiEta
...
OUTPUT_DIR = '/data/atlas/baines/20250322v5_Root/'
# OUTPUT_DIR = './tmp/'
...
def process_single_file(filepath, max_n_objs, shuffle_objs):
    ...
    x_part, x_event, y = read_file(filepath, 
                                max_num_particles=25, # This should be high enough that you never end up cutting off an object which is present in the truth matching. In reality, this just needs to be 25 for the samples I have
    ...
    class_outputs_lvbb = np.zeros((num_samps, 1))
    class_outputs_qqbb = np.zeros((num_samps, 1))
    ...
    if (num_samps>0):
        ...
        with torch.no_grad():
            for batch_idx in range(num_batches):
                ...
                class_outputs_lvbb[batch_idx*batch_size:(batch_idx+1)*batch_size] = class_outputs_batch_lvbb.cpu().numpy().argmax(axis=-1).reshape(-1, 1)
                class_outputs_qqbb[batch_idx*batch_size:(batch_idx+1)*batch_size] = class_outputs_batch_qqbb.cpu().numpy().argmax(axis=-1).reshape(-1, 1)
    ...
    if 0:
        return truth_label, dsid, event_weights, removals, mWh_qqbb, mWh_lvbb, mH_ll, eventNumber, class_outputs_lvbb, class_outputs_qqbb
    else:
        ...
        return {
            ...
            'll_mWh': mWh_ll,
            'll_mH': mH_ll,
            'll_class_outputs_lvbb': class_outputs_lvbb,
            'll_class_outputs_qqbb': class_outputs_qqbb,
    ...
    for channel in ['lvbb']:
        ...
        with uproot.recreate(output_path) as f:
            ...
            branches = {
                ...
                'll_mWh': "float32",
                'll_mH': "float32",
                'll_class_outputs_lvbb': "float32",
                'll_class_outputs_qqbb': "float32",
                'll_numbtags':"int64",
            ...
            for file_n, path in enumerate(all_files):
                data = process_single_file(filepath=path, max_n_objs=max_n_objs, shuffle_objs=SHUFFLE_OBJECTS)
