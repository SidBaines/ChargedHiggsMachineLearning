...
class HEPLoss(torch.nn.Module):
    ...
    def forward(self, inputs, targets, types, padding_token, n_objs, weights, log, cache, x_inputs=None):
        ...
        for layer in range(len([k for k in cache.store.keys() if 'attention' in k])):
            for head in range(cache.store[f'block_{layer}_attention']['attn_output_per_head'].shape[1]):
                # print(f"{layer}.{head}: {cache[f'block_{layer}_attention']['attn_output_per_head'][:,head,...].abs().mean(dim=-1)[...,0].mean()}") # Just the neutrinos
                absolute_activations_bo = (cache[f'block_{layer}_attention']['attn_output_per_head'][:,head,...] ** 2).mean(dim=-1) # Shape [batch object]
                total_abs_act += (einops.einsum(absolute_activations_bo * (types!=padding_token), 'batch object -> batch') / einops.einsum(types!=padding_token, 'batch object -> batch')).mean()
            if 'block_{layer}_post_attention' in cache.store.keys():
                absolute_activations_bo = (cache[f'block_{layer}_post_attention']['output'] ** 2).mean(dim=-1) # Shape [batch object]
                total_abs_act += (einops.einsum(absolute_activations_bo * (types!=padding_token), 'batch object -> batch') / einops.einsum(types!=padding_token, 'batch object -> batch')).mean()
        ...
        if log:
            wandb.log({
                ...
                "loss/activation": total_abs_act.item(),
